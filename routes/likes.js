...
const {
  createNotification,
} = require("../routes/functions/notificationCreator");
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      // 1️⃣ Get post data (we also need owner)
      const result = await pool.query(
        "SELECT liked_by, owner FROM posts WHERE id=$1",
        [post_id],
      );
      ...
      const { liked_by: likedByRaw, owner } = result.rows[0];
      ...
      // 3️⃣ Create notification ONLY if:
      // - It is a LIKE
      // - Liker is NOT the owner
      if (isLikeAction && owner !== userId) {
